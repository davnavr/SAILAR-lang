...
struct ModuleArena<'s> {
    modules: Vec<Box<Module<'s>>>,
    lookup: rustc_hash::FxHashMap<ModuleIdentifier, &'s Module<'s>>,
}
...
impl<'s> State<'s> {
    pub fn new() -> Self {
        Self { module_lookup: Default::default() }
    }

    pub fn force_load_module<S: crate::loader::Source>(&'s self, source: S) -> Result<Option<&'s Module<'s>>, S::Error> {
        let mut module_identifier = None;
        let mut module = Module::from_source(source, self, &mut module_identifier)?;
        ...
        unsafe fn push_get_module_unchecked<'s>(modules: &mut Vec<Box<Module<'s>>>, module: Box<Module<'s>>) -> &'s Module<'s> {
            let module_ptr = std::borrow::Borrow::borrow(&module) as *const Module<'s>;
            modules.push(module);
            &*module_ptr
        }
        ...
        let module_ref = if let Some(id) = module_identifier {
            match module_lookup.lookup.entry(id) {
                ...
                hash_map::Entry::Vacant(vacant) => unsafe {
                    let id_ptr = vacant.key() as *const ModuleIdentifier;
